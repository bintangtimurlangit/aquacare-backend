...
class AlertService {
  async checkMetrics(metrics, deviceId, deviceSettings = null) {
    ...
    // Check pH level
    if (
      metrics.ph_level < settings.phMin ||
      metrics.ph_level > settings.phMax
    ) {
      const alert = {
        type: 'warning',
        metric: 'ph_level',
        message:
          metrics.ph_level < settings.phMin
            ? ALERT_MESSAGES.ph_level.low
            : ALERT_MESSAGES.ph_level.high,
    ...
    // Check temperature
    if (
      metrics.temperature < settings.tempMin ||
      metrics.temperature > settings.tempMax
    ) {
      const alert = {
        type: 'warning',
        metric: 'temperature',
        message:
          metrics.temperature < settings.tempMin
            ? ALERT_MESSAGES.temperature.low
            : ALERT_MESSAGES.temperature.high,
